...
import { oddsLabel } from "@/lib/eventHelpers"
...
export type EventOutcome = {
  ...
  vol: number
  liquidity: number
  options: { id: string; label: string; shares: number }[]
...
function traded(o: EventOutcome) { return o.options.some((x) => x.shares > 0) }
function simCents(o: EventOutcome): number | null {
...
function optColor(label: string, i: number) {
  ...
  return SERIES_UNDER[i % SERIES_UNDER.length]
}

export default function EventTradeView({ outcomes, initialMarketId }: { outcomes: EventOutcome[]; initialMarketId: string }) {
  ...
  const held = selected.positions[opt.id] ?? 0
  const sellShares = held * (sellPct / 100)
  ...
  const heldValue = held > 0 && optPrice !== null ? Math.round(held * optPrice) : 0

  function pick(o: EventOutcome) {
  ...
  async function submit() {
    ...
    const res = await fetch("/api/apostas", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
  ...
  const sellDisabled = loading || held <= 0 || sellShares <= 0
  ...
  return (
    <div className="grid lg:grid-cols-[1fr_360px] gap-6 items-start">
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
        {outcomes.map((o, i) => {
          const cents = simCents(o)
          const on = o.marketId === selectedId
          return (
            <button key={o.marketId} onClick={() => pick(o)}
              className="flex items-center gap-3 w-full px-4 sm:px-5 py-3.5 text-left transition-colors hover:bg-[var(--card-2)]"
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[15px] font-semibold truncate" style={{ color: "var(--text-0)" }}>{o.name}</p>
                {o.secondary && <p className="text-[12px] truncate" style={{ color: "var(--text-2)" }}>{o.secondary}</p>}
              </div>
              <span className="hidden sm:block text-[12px] tabular-nums text-right" style={{ color: "var(--text-2)", width: 90 }}>{volLabel(o.vol)} vol</span>
              <span className="text-[13px] tabular-nums text-right" style={{ color: "var(--text-2)", width: 56 }}>{oddsLabel(cents, cents !== null)}</span>
      ...
      <div className="lg:sticky lg:top-6">
        {!authed ? (
          <div className="rounded-2xl p-6 text-center" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            ...
            <p className="text-xs mb-5 leading-relaxed" style={{ color: "var(--text-1)" }}>Entre para comprar e vender contratos com preço ao vivo.</p>
            <Link href="/login" className="block rounded-xl py-3.5 text-sm font-bold text-white" style={{ background: "linear-gradient(135deg, #00c076, #009e64)" }}>Entrar / Criar conta</Link>
          </div>
        ) : (
          <div className="rounded-2xl overflow-hidden" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            <div className="grid grid-cols-2" style={{ borderBottom: "1px solid var(--border)" }}>
              {(["BUY", "SELL"] as const).map((t) => (
                <button key={t} onClick={() => { setTab(t); setError("") }} className="py-3 text-sm font-bold transition-colors"
            ...
            <div className="p-5 space-y-4">
                ...
                {selected.options.map((o, i) => {
                  ...
                  return (
                    <button key={o.id} onClick={() => setOptionId(o.id)}
                      ...
                      style={active ? { background: `${clr}1f`, border: `1.5px solid ${clr}` } : { background: "var(--card-2)", border: "1.5px solid var(--border)" }}>
                      <p className={`font-bold ${binary ? "text-[10px] uppercase tracking-widest mb-1" : "text-sm"}`} style={{ color: active ? clr : binary ? "var(--text-2)" : "var(--text-0)" }}>{o.label}</p>
                      <p className={`font-bold tabular-nums leading-none ${binary ? "text-[26px]" : "text-[18px]"}`} style={{ color: active ? clr : "var(--text-0)" }}>{cents === null ? "—" : cents}¢</p>
                    </button>
                  )
                })}
              </div>
              ...
              {tab === "BUY" ? (
                <>
                  <div className="flex gap-1.5">
                    {[["R$10", 10], ["R$50", 50], ["R$100", 100], ["Máx", balance / 100]].map(([lbl, v]) => (
                      <button key={String(lbl)} onClick={() => setAmount(String(v))} className="flex-1 rounded-lg py-1.5 text-[11px] font-bold transition-all"
                  ...
                  <div className="relative">
                    ...
                    <input type="number" min="1" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0,00"
                      className="w-full rounded-xl pl-10 pr-4 py-3 text-sm font-semibold focus:outline-none" style={{ background: "var(--card-2)", border: "1.5px solid var(--border)", color: "var(--text-0)" }} />
                  </div>
                  {amountCents >= 100 && (
                ...
                <>
                  <div className="rounded-xl px-4 py-3 space-y-1.5" style={{ background: "var(--card-2)", border: "1px solid var(--border)" }}>
                    <div className="flex justify-between text-xs"><span style={{ color: "var(--text-2)" }}>Você tem</span><span className="font-semibold tabular-nums" style={{ color: "var(--text-0)" }}>{held.toFixed(2)} contratos {opt.label}</span></div>
                    <div className="flex justify-between text-xs"><span style={{ color: "var(--text-2)" }}>Valor atual</span><span className="font-semibold tabular-nums" style={{ color: "var(--text-0)" }}>{brl(heldValue)}</span></div>
                  </div>
                  {held > 0 && (
                    <div className="flex gap-1.5">
                      {[25, 50, 75, 100].map((p) => (
                        <button key={p} onClick={() => setSellPct(p)} className="flex-1 rounded-lg py-1.5 text-[11px] font-bold transition-all"
                  ...
                  {sellShares > 0 && (
                    <div className="rounded-xl px-4 py-3 flex justify-between items-center" style={{ background: "var(--card-2)", border: "1px solid var(--border)" }}>
                      <span className="text-xs" style={{ color: "var(--text-2)" }}>Você recebe</span><span className="text-sm font-bold tabular-nums" style={{ color: "var(--green)" }}>{brl(sellProceeds)}</span>
                    </div>
                  )}
                </>
              )}

              {error && <div className="rounded-xl px-4 py-2.5 text-xs" style={{ background: "var(--red-dim)", color: "var(--red)", border: "1px solid rgba(255,77,77,0.2)" }}>{error}</div>}
              ...
              <button onClick={submit} disabled={tab === "BUY" ? buyDisabled : sellDisabled}
                ...
                {loading ? "Processando..." : tab === "BUY" ? `Comprar ${opt.label}${amountCents >= 100 ? ` · ${brl(amountCents)}` : ""}` : held <= 0 ? `Sem contratos ${opt.label}` : `Vender ${opt.label} · ${brl(sellProceeds)}`}
              </button>
